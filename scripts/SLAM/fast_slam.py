...
class Particle:
    ...
    def updateEstimatedPose(self, currentRawReading):
        ...
        if rawMove > 0.3:
            if self.prevRawMovingTheta != None and self.prevMatchedMovingTheta != None:
                if rawYMove > 0:
    ...
    def updateTrajectory(self, matchedReading):
        ...
        x, y, theta = matchedReading['x'], matchedReading['y'], matchedReading['theta']
    ...
    def plotParticle(self):
        ...
        self.og.plotOccupancyGrid([-5000, 5000], [-5000, 5000], plotThreshold=False)


def process_sensor_data(pf, sensorData, save_dir='test_data', plot_trajectory=True):
    ...
    for key in sorted(sensorData.keys()):
        ...
        xRange, yRange = [-5000, 5000], [-5000, 5000]
        ...
        plt.savefig(f'{save_dir}/' + str(count).zfill(3) + '.png')
        plt.close()
...
def main():
    ...
    import pickle
    filepath = "test_data/preprocessed_data/test.pkl"
